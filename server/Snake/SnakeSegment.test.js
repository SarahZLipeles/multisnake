...
describe("SnakeSegment", () => {
  ...
  beforeEach("create a new snake segment", () => {
    testSegment = new SnakeSegment(...position, "next test", "prev test");
  });

  it("has x, y, z, and next properties", () => {
    expect(testSegment).to.have.keys("x", "y", "z", "next", "prev");
  });
  ...
  it("has default property values of 0 and null next value", () => {
    ...
    expect(newSnakeSegment).to.deep.equal({ x: 0, y: 0, z: 0, next: null, prev: null });
  });

  it("clones a new snake segment", () => {
    const clonedEntity = testSegment.clone();
    expect(clonedEntity).to.deep.equal({ x: position[0], y: position[1], z: position[2], next: "next test", prev: "prev test" });
  });
});
